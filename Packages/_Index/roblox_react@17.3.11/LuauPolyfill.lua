local REQUIRED_MODULE = require(script.Parent.Parent["roblox_luau-polyfill@1.2.5"]["luau-polyfill"])
export type Array<T> = REQUIRED_MODULE.Array<T>
export type AssertionError = REQUIRED_MODULE.AssertionError 
export type Error = REQUIRED_MODULE.Error 
export type Map<T, V> = REQUIRED_MODULE.Map<T, V>
export type Object = REQUIRED_MODULE.Object 
export type PromiseLike<T> = REQUIRED_MODULE.PromiseLike<T>
export type Promise<T> = REQUIRED_MODULE.Promise<T>
export type Set<T> = REQUIRED_MODULE.Set<T>
export type Symbol = REQUIRED_MODULE.Symbol 
export type Timeout = REQUIRED_MODULE.Timeout 
export type Interval = REQUIRED_MODULE.Interval 
export type WeakMap<T, V> = REQUIRED_MODULE.WeakMap<T, V>
return REQUIRED_MODULE
