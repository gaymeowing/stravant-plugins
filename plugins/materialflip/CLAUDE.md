...
1. **Functionality layer** — Targeting, hover feedback, the flip operation.
   ...
   - `src/getMeshRepresentation.lua` — Creates MeshParts for mesh classes by cloning templates embedded in the plugin as a single serialized+base64 blob (`src/meshTemplateBlob.lua`, ~9.6 KB, generated by `tools/generateMeshTemplateBlob.lua`) - no asset download yield on first use. Falls back to `CreateMeshPartAsync` from the published asset if a template is missing.
