...
function RotateHandles:render(hoveredHandleId)
	...
	if self._draggingHandleId and self._handles[self._draggingHandleId] then
		...
		-- Show companion handles (e.g. move arrows) alongside rotate during drag
		if self._props.CompanionHandles
			and self._props.ShowCompanionWhileDragging
			and self._props.ShowCompanionWhileDragging()
		then
			children.CompanionHandles = self._props.CompanionHandles:render(nil)
		end
...
function RotateHandles:_updateHandles()
	if self._selectionInfo:isEmpty() or not self._props.Visible() then
		...
		for handleId, handleDefinition in pairs(RotateHandleDefinitions) do
			...
			local isPrimary = primaryAxis and localAxis:FuzzyEq(primaryAxis) or false
			local lastAxisSigned = primaryAxis and primaryAxis:Cross(localAxis) or Vector3.zero
